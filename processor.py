...
BANNED = ['Aaron Wiggins', 'Isaiah Joe', 'Domantas Sabonis', 'Haywood Highsmith', 'Jonas Valančiūnas', 'Royce O\'Neale', 'Bennedict Mathurin', 'Jrue Holiday', 'Jaylen Brown', 'Nikola Vučević']
CSV_FIELDNAMES = ["Player", "Market", "Predicted", "Buffer", "Line", "Rank", "Last Ten", "Odds", "Game"]
...
def append_to_text(row, filename=TEXT_OUTPUT_FILE):
    ...
    formatted_line = f"🏀 {game}: {player} {direction} {line_value} {market_formatted} ({odds} FD)"
    try:
        ...
        for prop_entry in entries_list:
            if isinstance(prop_entry, dict):
                ...
                if player_name:
                    ...
                    print(f"Warning: Prop entry missing 'player' key in {bookmaker_name}: {prop_entry}")
            else:
                 print(f"Warning: Found non-dictionary prop entry in {bookmaker_name}: {prop_entry}")
...
def process_player_entry(entry, consistent_players_map, current_best_rank, current_best_row, conn):
    try:
        player = entry['player']
        market = entry['market']
        ...
        if rank < current_best_rank and last_ten_trend_is_over:
            ...
            if MODEL_RECENCY:
                ...
                predicted_stat, error = run_xgb(player, team, opponent, home_or_away, market, 20)
            
            is_good_over_bet = predicted_stat is not None and (math.ceil(predicted_stat) >= line + error)
