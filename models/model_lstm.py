    ...
    try:
        ...
        def aggregate_position_data(json_data, exclude_player):
            ...
            for player_name, value in json_data.items():
                pos = positions_df.loc[positions_df['player'] == player_name, 'pos'].values
                if pos.size > 0:
                    position_totals[pos[0]] += value

            return position_totals
        
        stats_fields = ['teammates_points', 'teammates_rebounds', 'teammates_assists', 
...
def get_soft_predictions(team, opp, player_df, player):
    ...
    def populate_player_stats(players, stats, team_of_player):
        for player_name in players:
            ...
            for key in stats:
                try:
                    predicted_value = soft(player_name, opp if team_of_player == team else team, key, 1)
                    if pd.isna(predicted_value):
                        predicted_value = player_df.loc[player_df['player'] == player_name, f"avg_{key}"].values
...
def rolling_lstm_train(player, market, conn, feature_weights=None):
    ...
    for i in range(min_train_size + seq_length, len(X_processed)):
        ...
        model = LSTMWithAttention(input_size, hidden_size=64, num_layers=2, dropout=0.3).to(device)
    ...
    final_model = LSTMWithAttention(X_processed.shape[1], hidden_size=64, num_layers=2, dropout=0.3).to(device)
    criterion = nn.MSELoss()
...
if __name__ == "__main__":
    ...
    prediction, error = run_lstm(player_name, team_name, opp_name, 0, market_name, 20, feature_weights)
