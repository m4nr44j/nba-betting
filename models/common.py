...
def load_game_stats(player, conn):
    ...
    try:
        ...
        for stat_field in stats_fields:
            df[stat_field] = df[stat_field].apply(lambda x: aggregate_position_data_game_stats(x, player, positions_df))
...
def get_soft_predictions(team, opp, player_df, player):
    ...
    def populate_player_stats(players, stats, team_of_player):
        ...
        for player in players:
            ...
            for key in stats:
                predicted_value = predict_player_stat(player, opp if team_of_player == team else team, key, 1)
    ...
    results = {
        'team': [team],
        'opp': [opp],
        'teammates_points': [aggregate_position_data_predictions(team_stats['pts'], player_df)],
        'teammates_rebounds': [aggregate_position_data_predictions(team_stats['trb'], player_df)],
        'teammates_assists': [aggregate_position_data_predictions(team_stats['ast'], player_df)],
        'opponents_points': [aggregate_position_data_predictions(opp_stats['pts'], player_df)],
        'opponents_rebounds': [aggregate_position_data_predictions(opp_stats['trb'], player_df)],
        'opponents_assists': [aggregate_position_data_predictions(opp_stats['ast'], player_df)],
        'teammates_turnovers': [aggregate_position_data_predictions(team_stats['tov'], player_df)],
        'opponents_blocks': [aggregate_position_data_predictions(opp_stats['blk'], player_df)],
        'opponents_turnovers': [aggregate_position_data_predictions(opp_stats['tov'], player_df)]
    }
