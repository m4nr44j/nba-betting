...
from models.common import (
    load_nba, load_player_positions, load_game_stats, 
    get_last_data, get_soft_predictions, get_recent_team_form
)
import os
...
def gradient_boost_recency(player, market, conn, n_estimators, feature_weights=None):
    ...
    features = [col for col in df.columns if col not in ['date', market, 'days_since', 'sample_weight']]
...
def run_recency(player, team, opp, hoa, market, nestimators, feature_weights=None):
    ...
    if avg_mp < 10:
        return 0, 0
    pipeline, error = gradient_boost_recency(player, market, conn, nestimators, feature_weights)
    ...
    if feature_weights:
        numerical_features = [
            ...
            'team_recent_pts', 'team_recent_plus_minus', 'team_recent_pace',
            'opp_recent_pts_allowed', 'opp_recent_plus_minus', 'opp_recent_pace'
        ]
    ...
    if 'team_recent_plus_minus' in team_form and 'opp_recent_plus_minus' in team_form:
        form_diff = team_form['team_recent_plus_minus'] - team_form['opp_recent_plus_minus']
...
if __name__ == "__main__":
    ...
    feature_weights = {
        'mp': 2.5, 'plus_minus': 1.8, 'opp': 3.0,
        'teammates_points_G': 1.3, 'teammates_points_F': 1.4, 'teammates_points_C': 1.2,
        'team_recent_plus_minus': 1.2, 'opp_recent_plus_minus': 1.3
    }

    prediction, error = run_recency(player_name, team_name, opp_name, 0, market_name, 20, feature_weights)
