...
def game_ids(commence_time_to: str):
    formatted_time = commence_time_to.replace('+00:00', 'Z') \
        if '+00:00' in commence_time_to else commence_time_to
...
def data_update():
    ...
    try:
        ...
        date_col = "date" if "date" in df_sql.columns else ("Date" if "Date" in df_sql.columns else None)
        if date_col is None:
            return
    ...
    mask = (df_full["Date_dt"] >= pd.Timestamp(start_dt)) & (df_full["Date_dt"] <= pd.Timestamp(end_dt))
