    ...
    try:
        ...
        if not isinstance(data, list) or not data:
            ...
            if not all(col in df.columns for col in required_cols):
                raise ValueError(
                    f"Missing required columns in API response. Found: {df.columns}. Needed: {required_cols}"
                )

            df_final = df[required_cols].copy()
            df_final["player"] = df_final["player"].replace(
                "Jimmy Butler", "Jimmy Butler III"
            )

            team_grouped = (
                df_final.groupby("team")
                .apply(
                    lambda group: group[["player", "status"]].to_dict(orient="records"),
