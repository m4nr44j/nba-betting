    ...
    for platform, markets in data.items():
        ...
        for market, props in markets.items():
            ...
            if not feature_column:
                print(f"Warning: Skipping unmapped market '{market}' for platform '{platform}'")
                continue 

            for prop in props:
                if not all(k in prop for k in ["description", "game_id", "point", "price"]):
                    print(f"Warning: Skipping prop due to missing keys: {prop}")
                    continue

                if lowest_price_threshold <= prop["price"] <= highest_price_threshold and prop["point"] > 2.5:
                    key = (prop["description"], prop["game_id"])
                    current_line = prop["point"]
                    current_price = prop["price"]

                    if key in best_props_for_market:
                        if current_line < best_props_for_market[key]['line'] and lowest_price_threshold <= current_price <= highest_price_threshold:
