...
def load_data(cursor):
    ...
    with open(csv_path, 'r') as f:
        next(f)  # Skip header
        cursor.copy_from(f, 'nba_append', sep=',', null='None', columns=(
            'player', 'date', 'age', 'team', 'hoa', 'opp', 'result', 'gs', 'mp', 'fg', 'fga',
            'fg_percent', 'twop', 'twopa', 'twop_percent', 'tpm', 'threepa', 'threep_percent',
            'ft', 'fta', 'ft_percent', 'ts_percent', 'orb', 'drb', 'trb', 'ast', 'stl', 'blk', 
            'tov', 'pf', 'pts', 'gmsc', 'bpm', 'plus_minus', 'pos', 'player_additional'
...
def update_game_stats(cursor):
    ...
    for game_date, team, opponent in new_games:
        ...
        opponent_stats = {'points': {}, 'rebounds': {}, 'assists': {}, 'blocks': {}, 'turnovers': {}}
        ...
        for relation, team_to_query in [('teammates', team), ('opponent', opponent)]:
            ...
            if not stats_for_game:
                print(f"Warning: No player stats found for {relation} ({team_to_query}) on {game_date}.")
        ...
        rows_to_insert.append([
            ...
            Json(team_stats['turnovers']), Json(opponent_stats['blocks']), Json(opponent_stats['turnovers'])
        ])
...
def create_most_recent_player_team_table(cursor):
    ...
    for player, pos in PLAYER_POSITIONS.items():
        cursor.execute("UPDATE public.nba SET pos = %s WHERE player = %s;", (pos, player))
