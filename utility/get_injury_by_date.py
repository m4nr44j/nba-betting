...
TEAM_ABBR = {
    ...
    "Houston Rockets": "HOU", "Los Angeles Clippers": "LAC", "LA Clippers": "LAC",
    "Los Angeles Lakers": "LAL", "Memphis Grizzlies": "MEM", "Minnesota Timberwolves": "MIN",
    "New Orleans Pelicans": "NOP", "Oklahoma City Thunder": "OKC", "Phoenix Suns": "PHX",
        ...
        for page_num in range(len(doc)):
            ...
            while i < len(lines):
                ...
                if current_team and i + 2 < len(lines):
                    # Skip header lines
                    if line in ['Player Name', 'Current Status', 'Reason', 'Game Date', 'Game Time', 'Matchup', 'Team']:
                        i += 1
                        continue
                    
                    # Check if this might be a player name (contains comma or has proper format)
                    if (',' in line and any(c.isalpha() for c in line)) or (line.replace(' ', '').replace("'", "").isalnum() and len(line) > 3):
                        player_name = line
                        ...
                        if potential_status in ['Out', 'Questionable', 'Probable', 'Doubtful', 'Available']:
                            ...
                            normalized_status = normalize_injury_status(potential_status)
