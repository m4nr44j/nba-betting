...
After running the main application and generating picks, you can use the prop tracker to monitor the performance of your bets in real-time.

### Running the Prop Tracker

The prop tracker continuously monitors your selected props and displays updates as games progress. It requires that `output.txt` exists (generated by `run.py`).
...
The tracker will:
- Monitor props from `output.txt`
- Display real-time updates as games progress
- Show current stats vs. the betting line
- Update automatically as new game data becomes available
